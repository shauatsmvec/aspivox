...
export function VaporizeTextCycle({
  ...
  alignment = "center",
  tag = Tag.P,
  shouldAnimateAlways = true,
}: VaporizeTextCycleProps & { shouldAnimateAlways?: boolean }) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const wrapperRef = useRef<HTMLDivElement | null>(null);
  const observerInView = useIsInView(wrapperRef as React.RefObject<HTMLElement>);
  const isInView = shouldAnimateAlways || observerInView;
  ...
  const fontConfig = useMemo(() => {
    // Robust parsing for font size - extract digits only
    const sizeMatch = font.fontSize?.match(/\d+/);
    const fontSize = sizeMatch ? parseInt(sizeMatch[0]) : 50;
