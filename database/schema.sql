...
INSERT INTO courses (title, slug, description, is_free, is_active, start_date) VALUES
('Python Programming', 'python', 'Master Python from basics to advanced with real projects.', false, true, '2026-06-01'),
('Java', 'java', 'Build robust applications with Java under expert guidance.', false, true, '2026-06-15'),
('C & C#', 'c-csharp', 'Strong programming fundamentals for every developer.', false, true, '2026-07-01'),
('React JS', 'react', 'Build dynamic frontends with the world''s most popular library.', false, true, '2026-06-10'),
('MERN Stack', 'mern', 'Full-stack web development: MongoDB, Express, React, Node.', false, true, '2026-07-15'),
('Data Science', 'data-science', 'Learn data analysis, visualization, and ML basics.', false, true, '2026-08-01'),
('IoT', 'iot', 'Connect the physical world with smart technology.', false, true, '2026-08-15'),
('Generative AI', 'generative-ai', 'Explore the future of AI with hands-on GenAI projects.', false, true, '2026-09-01'),
('PHP & Web Basics', 'php', 'Build web apps and understand server-side programming.', false, true, '2026-09-15');
...
-- Admin Policies (Checks is_admin flag in students table)
CREATE POLICY "Admin students" ON students FOR ALL USING (
  EXISTS (SELECT 1 FROM students WHERE id = auth.uid() AND is_admin = true)
  OR auth.jwt() ->> 'email' = 'shahid.aspivox@zohomail.in'
);
CREATE POLICY "Admin enrollments" ON enrollments FOR ALL USING (
  EXISTS (SELECT 1 FROM students WHERE id = auth.uid() AND is_admin = true)
  OR auth.jwt() ->> 'email' = 'shahid.aspivox@zohomail.in'
);
CREATE POLICY "Admin applications" ON internship_applications FOR ALL USING (
  EXISTS (SELECT 1 FROM students WHERE id = auth.uid() AND is_admin = true)
  OR auth.jwt() ->> 'email' = 'shahid.aspivox@zohomail.in'
);
CREATE POLICY "Admin contacts" ON contact_submissions FOR ALL USING (
  EXISTS (SELECT 1 FROM students WHERE id = auth.uid() AND is_admin = true)
  OR auth.jwt() ->> 'email' = 'shahid.aspivox@zohomail.in'
);
CREATE POLICY "Admin stats" ON site_stats FOR ALL USING (
  EXISTS (SELECT 1 FROM students WHERE id = auth.uid() AND is_admin = true)
  OR auth.jwt() ->> 'email' = 'shahid.aspivox@zohomail.in'
);
CREATE POLICY "Admin courses" ON courses FOR ALL USING (
  EXISTS (SELECT 1 FROM students WHERE id = auth.uid() AND is_admin = true)
  OR auth.jwt() ->> 'email' = 'shahid.aspivox@zohomail.in'
);
CREATE POLICY "Admin team" ON team_members FOR ALL USING (
  EXISTS (SELECT 1 FROM students WHERE id = auth.uid() AND is_admin = true)
  OR auth.jwt() ->> 'email' = 'shahid.aspivox@zohomail.in'
);
